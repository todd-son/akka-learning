...
import actor.ch01.iot.DeviceGroupQuery.CollectionTimeout
...
class DeviceGroupQuery (
  ...
  override def preStart(): Unit = actorToDeviceId.keySet.foreach(
    deviceActor => {
  ...
  def waitingForReplies(
    ...
    case Terminated(deviceActor) =>
      receivedResponse(deviceActor, DeviceGroup.DeviceNotAvailable, stillWating, repliesSoFar)
